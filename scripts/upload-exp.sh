...
#
# Usage ./upload-exp.sh case_path syz_repro_url ssh_port image_path syz_commit type
# EXITCODE: 2: syz-execprog supports -enable. 3: syz-execprog do not supports -enable.

set -ex

if [ $# -ne 7 ]; then
  echo "Usage ./upload-exp.sh case_path syz_repro_url ssh_port image_path syz_commit type, c_repro"
  exit 1
fi
